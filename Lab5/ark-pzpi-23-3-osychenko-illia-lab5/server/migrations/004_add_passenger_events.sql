...
-- Додаємо події для першого завершеного рейсу
-- Використовуємо динамічний вибір ID замість хардкоду
INSERT INTO passenger_events (trip_id, event_type, timestamp, latitude, longitude, passenger_count_after, device_local_id, is_synced)
SELECT 
    (SELECT id FROM trips WHERE status = 'completed' ORDER BY id LIMIT 1) as trip_id,
    'entry' as event_type,
...
-- Всі пасажири виходять в кінці першого завершеного рейсу
INSERT INTO passenger_events (trip_id, event_type, timestamp, latitude, longitude, passenger_count_after, device_local_id, is_synced)
SELECT 
    (SELECT id FROM trips WHERE status = 'completed' ORDER BY id LIMIT 1) as trip_id,
    'exit' as event_type,
...
-- Додаємо події для першого рейсу в процесі
-- Використовуємо динамічний вибір ID замість хардкоду
INSERT INTO passenger_events (trip_id, event_type, timestamp, latitude, longitude, passenger_count_after, device_local_id, is_synced)
SELECT 
    (SELECT id FROM trips WHERE status = 'in_progress' ORDER BY id LIMIT 1) as trip_id,
    'entry' as event_type,
...
-- Додаємо події для рейсу на посадці
-- Використовуємо динамічний вибір ID замість хардкоду
INSERT INTO passenger_events (trip_id, event_type, timestamp, latitude, longitude, passenger_count_after, device_local_id, is_synced)
SELECT 
    (SELECT id FROM trips WHERE status = 'boarding' ORDER BY id LIMIT 1) as trip_id,
    'entry' as event_type,
