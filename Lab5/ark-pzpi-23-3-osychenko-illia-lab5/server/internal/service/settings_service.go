...
type SettingsService interface {
	...
	ImportSettings(ctx context.Context, export *SettingsExport, userID int64) error
}

// SettingsExport структура для експорту/імпорту налаштувань
...
// ExportSettings експортує поточні налаштування у форматі JSON
...
// ImportSettings імпортує налаштування з експортованого файлу
func (s *settingsService) ImportSettings(ctx context.Context, export *SettingsExport, userID int64) error {
	if export == nil || export.Settings == nil {
		return fmt.Errorf("invalid export data: settings are empty")
	}
	...
	if err := s.ValidateSettings(export.Settings); err != nil {
		return fmt.Errorf("imported settings validation failed: %w", err)
	}

	if err := s.UpdateSettings(ctx, export.Settings, userID); err != nil {
		return fmt.Errorf("failed to apply imported settings: %w", err)
	}
