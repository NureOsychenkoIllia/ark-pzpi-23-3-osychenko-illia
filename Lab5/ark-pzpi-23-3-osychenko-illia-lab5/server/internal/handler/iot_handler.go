...
type IoTHandler struct {
	...
	auditHelper *middleware.AuditHelper
}

func NewIoTHandler(iotService service.IoTService, auditHelper *middleware.AuditHelper) *IoTHandler {
	return &IoTHandler{
...
func (h *IoTHandler) SyncEvents(c *fiber.Ctx) error {
	...
	// Логування аудиту для IoT пристрою
	if h.auditHelper != nil {
		h.auditHelper.LogDeviceAction(c, "SYNC", "passenger_events", strconv.FormatInt(req.TripID, 10), map[string]any{
			"trip_id":      req.TripID,
...
func (h *IoTHandler) SendPriceRecommendation(c *fiber.Ctx) error {
	...
	// Логування аудиту для IoT пристрою
	if h.auditHelper != nil {
		h.auditHelper.LogDeviceAction(c, "CREATE", "price_recommendation", strconv.FormatInt(req.TripID, 10), map[string]any{
			"trip_id":           req.TripID,
			"base_price":        req.BasePrice,
			"recommended_price": req.RecommendedPrice,
			"occupancy_rate":    req.OccupancyRate,
			"demand_coeff":      req.DemandCoefficient,
			"time_coeff":        req.TimeCoefficient,
			"day_coeff":         req.DayCoefficient,
		})
	}
...
func (h *IoTHandler) GetTripConfig(c *fiber.Ctx) error {
	...
	// Логування аудиту для IoT пристрою
	if h.auditHelper != nil {
		h.auditHelper.LogDeviceAction(c, "READ", "trip_config", strconv.FormatInt(tripID, 10), map[string]any{
			"trip_id":      tripID,
			"bus_capacity": config.BusCapacity,
